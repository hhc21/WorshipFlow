...
## 0) 최신 현황 스냅샷 (2026-03-04)

| 구분 | 개수 | 상태 |
|---|---:|---|
| 기능 항목(F-*) | 34 | 전체 `DONE` |
| 안정성/보안(S-*) | 8 | 전체 `DONE` |
| 부분완료 | 6 | LiveCue/Safari 안정화 트랙 진행 중 |
| 미완료 | 0 | 없음 |
...
LiveCue 실사용 상태:
- iPad Safari 기준으로 `악보 표시 + 필기 + 곡 전환` 안정화가 재오픈됨
- `plan.md` 13장과 동기화하여 LiveCue 관련 항목은 `PARTIAL/IN-PROGRESS`로 유지
...
| F-306 | 스와이프로 곡 전환(태블릿) | PARTIAL | iPad Safari 실기기에서 연속 10분 검증 필요 |
| F-307 | 전체화면에서 악보만 집중 표시 | IN-PROGRESS | 일부 환경에서 회색/검은 화면 재현 이슈 추적 중 |
| F-308 | 확대/축소(핀치줌) 안정 동작 | PARTIAL | 필기 모드와 동시 사용 시 회귀 확인 필요 |
...
| F-403 | 펜슬/드로잉 입력(태블릿) | IN-PROGRESS | iPad Safari에서 펜슬 필기 안정화 재검증 중 |
...
| S-003 | CORS/토큰 URL 관련 로드 실패 방지 | IN-PROGRESS | Safari Firestore/Storage 경로 재검증 진행 중 |
| S-004 | 비로그인/권한없음 라우트 보호 | DONE | 직접 URL 접근 시 가드 동작 |
| S-005 | Firestore/Storage rules 최소권한 적용 | DONE | 규칙 리뷰 + 실제 접근 테스트 |
| S-006 | 불필요한 과다 구독/읽기 최적화 | PARTIAL | LiveCue 웹 실기기에서 Listen 경로 추가 확인 필요 |
